...
onready var hitbox: Area2D = get_node("Node/Sprite/Hitbox")
onready var charge_particles: Particles2D = get_node("Node/Sprite/Hitbox/CollisionShape2D/ChargeParticles")
...
func interpolate_pos(initial_pos: Vector2, final_pos: Vector2) -> void:
	var __ = tween.interpolate_property(self, "position", initial_pos, final_pos,
	0.8, Tween.TRANS_QUART, Tween.EASE_OUT)
	assert(__)
	__ = tween.start()
	assert(__)
